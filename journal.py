...
def clean_df(df, ordered_moods=ORDERED_MOODS, scale=SCALE):
    ...
    df_with_one_hot_encoding, all_activities = convert_activities_to_categorical(
        df)
...
def gen_dot_plot(df, ordered_moods=ORDERED_MOODS, matrix_length=30):
    ...
    for x in range(matrix_height):
        new_row = mood_list_with_remainder[(x * matrix_length):((x + 1) *
                                                                matrix_length)]
    ...
    colormap = colors.ListedColormap([
        "darkgreen", "forestgreen", "limegreen", "yellowgreen", "greenyellow",
        "darkgrey"
    ])
...
def mood_to_score(df, ordered_moods=ORDERED_MOODS, scale=SCALE):
    ...
    for mood in original_metric.keys():
        ...
        weighted_score = scale / (old_max - old_min) * (value -
                                                        old_max) + scale
...
def gen_hist(df, ordered_moods=ORDERED_MOODS):
